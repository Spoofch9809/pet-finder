...
def list_photos(db: Session = Depends(get_session)):
    return services.get_all_photos(db)

@router.get("/{photo_id}", response_model=schema.PetPhotoResponse)
def get_photo(photo_id: int, db: Session = Depends(get_session)):
...
def create_photo(photo_in: schema.PetPhotoCreate, db: Session = Depends(get_session)):
