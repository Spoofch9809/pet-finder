...
def list_breeds(db: Session = Depends(get_session)):
    return services.get_all_breeds(db)

@breed_router.get("/{breed_id}", response_model=schema.BreedResponse)
def get_breed(breed_id: int, db: Session = Depends(get_session)):
...
def create_breed(breed_in: schema.BreedCreate, db: Session = Depends(get_session)):
    return services.create_breed(db, breed_in)

@breed_router.put("/{breed_id}", response_model=schema.BreedResponse)
def update_breed(breed_id: int, breed_in: schema.BreedBase, db: Session = Depends(get_session)):
...
def delete_breed(breed_id: int, db: Session = Depends(get_session)):
