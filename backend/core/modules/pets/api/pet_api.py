...
def list_pets(db: Session = Depends(get_session)):
    return services.get_all_pets(db)

@router.get("/{pet_id}", response_model=schema.PetResponse)
def get_pet(pet_id: int, db: Session = Depends(get_session)):
...
def create_pet(pet_in: schema.PetCreate, db: Session = Depends(get_session)):
    return services.create_pet(db, pet_in)

@router.put("/{pet_id}", response_model=schema.PetResponse)
def update_pet(pet_id: int, pet_in: schema.PetUpdate, db: Session = Depends(get_session)):
