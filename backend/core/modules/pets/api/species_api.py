...
def list_species(db: Session = Depends(get_session)):
    return services.get_all_species(db)

@router.get("/{species_id}", response_model=schema.SpeciesResponse)
def get_species(species_id: int, db: Session = Depends(get_session)):
...
def create_species(species_in: schema.SpeciesCreate, db: Session = Depends(get_session)):
    return services.create_species(db, species_in)

@router.put("/{species_id}", response_model=schema.SpeciesResponse)
def update_species(species_id: int, species_in: schema.SpeciesBase, db: Session = Depends(get_session)):
...
def delete_species(species_id: int, db: Session = Depends(get_session)):
