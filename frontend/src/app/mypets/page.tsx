  ...
  return (
    <section className="p-3">
      <div className="row g-3">
        ...
        {pets.map((p) => {
          const hasPhoto = !!p.photoUrl?.trim();
          return (
            <div className="col-md-6 col-xl-4" key={p.id}>
              <div className="card h-100">
                <div className={`${styles.petCardMedia} card-img-top`}>
                  {hasPhoto ? (
                    <img className={styles.petCardImgContain} src={p.photoUrl} alt={p.name}/>
                  ) : (
                    <div className={styles.petCardPlaceholder}>
                      <i className="bi bi-image fs-2" aria-hidden="true"/>
                      <span className="small">No photo yet</span>
                    </div>
                  )}
                </div>
